...
use cyllenea\AmbulanceConnector\Exception\NoMedicalCheckUpDatesException;
use cyllenea\AmbulanceConnector\Exception\WorkplaceNotFoundException;
		...
		if ($workplaceCache === NULL) {
			if ($response = $this->call("vratSeznamPracovist")) {
				...
				foreach ($workplaces as $workplace) {
					...
					$controlType = array_key_exists(0, $examinations) ? $examinations[0]->getId() : '001';
					$wp = new Workplace($workplace->id, $workplace->nazev, $examinations, $controlType);
					$output[] = $wp;
				}
			...
			if ($response = $this->call("vratSeznamTerminuPracoviste", $workplace->getId(), $workplace->getControlType(), $startDate->getTimestamp(), $endDate->getTimestamp())) {
				...
				foreach ($output as $date => $exa) {
					if (!$output[$date] instanceof Workplace) {
						if(in_array($workplace->getId(), $this->nonReducedWorkplaces)) {
							$workplace->setReduce(false);
						}
